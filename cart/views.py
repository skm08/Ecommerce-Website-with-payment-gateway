    ...
    def post(self,*args,**kwargs):
        ...
        if coupon.exists():
            ...
            if current_date > expiry_date:
                messages.warning(self.request,"The coupon has expired!")
                return redirect('cart')
            
            if current_date < active_date:
                messages.warning(self.request, "The coupon is yet to availavle!")
                return redirect('cart')
            
            if cart.total() < coupon.required_amount_to_use_coupon:
                messages.warning(self.request, f"You have to shop at least {coupon.required_amount_to_use_coupon} to use coupon.")
                return redirect('cart')
